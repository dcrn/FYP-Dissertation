...
For both projects, Flask and its dependencies (itsdangerous, Jinja2, MarkupSafe and Werkzeug) were installed using \textbf{pip} in the virtualenv, along with the libraries specific to each project as seen below:
...
Components.register('mesh', MeshComponent, {
	...
	\subsubsection{Selection Object.}
	The selection object is a class based the Three.js Mesh object that consists of a wireframe box to outline the selected entity, and three `handles'; one for each axis. It was created to allow the user to click and drag entities around the scene rather than having to change the position values in the properties editor.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{awsnetwork}
